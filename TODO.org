...
:LOGBOOK:
CLOCK: [2026-01-04 Sun 12:20]
CLOCK: [2026-01-04 Sun 11:51]--[2026-01-04 Sun 12:01] =>  0:10
:END:
If Cytraco has a trained configured, and can connect to it, continue without any interactions. Otherwise, offer to retry, exit or continue in demo mode:
...
When it finds multiple trainers, allow the user to select one by entering the number corresponding to the list of trainers. Once the user has selected one, continue as if one was found:
